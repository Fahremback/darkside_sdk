...
// Offset dinâmico para swapchain - atualizado via pattern scanning para 2024-2026
static uintptr_t g_swapchain_offset = 0;

static void initialize_swapchain_offset( ) {
static bool initialized = false;
if ( !initialized ) {
// Pattern para encontrar offset da swapchain no DX12/Vulkan
auto pattern = g_opcodes->scan_absolute( g_modules->m_modules.rendersystem_dll.get_name( ), "48 8B 0D ? ? ? ? 48 8B 01 FF 90 ? ? ? ?", 0x3 );
g_swapchain_offset = pattern ? *reinterpret_cast<int32_t*>( pattern ) : 0x170;
initialized = true;
}
}
